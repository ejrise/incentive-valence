...
```
incentive-valence/data/raw/behavioral/fmri_task/
...
```
incentive-valence/scripts/preprocessing/fMRI/masks
```
Folder of common ROI masks used across analyses for Experiments 1 and 2. If a script calls for an ROI mask, it should be loaded from here. 

```
incentive-valence/scripts/preprocessing/fMRI/Task
```
...
```
incentive-valence/scripts/preprocessing/fMRI/7T
```
...
- `denoised_funcs_to_mni.sh`: Normalizes denoised BOLD images to MNI space. After this, the MNI normalized denoised BOLD images, MNI normalized T1 images, motion outier info, ROI masks, and physio regressors are ready to be imported into CONN_Toolbox for functional connectivity estimation (see Supplementary Info methods for this process).


```
incentive-valence/scripts/preprocessing/behavioral/makeDF_wFSs.py
```
...
```
incentive-valence/scripts/analysis/descriptives/
...
```
incentive-valence/scripts/paradigm/
    IV_task.py
    IV_functions.py
    IV_practice.py
    IV_compute_bonus.py
...
```
incentive-valence/scripts/analysis/fMRI/make_GLM_timing_files/
...
```
incentive-valence/scripts/analysis/fMRI/connectome_plots/
...
```
incentive-valence/scripts/analysis/fMRI/bayes_studentT_of_L2s/
