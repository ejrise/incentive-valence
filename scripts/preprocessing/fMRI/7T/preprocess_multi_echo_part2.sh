...
echo "The mag brain file is: $mag_brain"
...
#average the 3 echoes to get avg SBRef script 
...
bet $SBRef $sub_dir/$sub"_SBRef_avg_brain" -f 0.40 -g 0.25 -m 
fi

SBRef_brain=$sub_dir/$sub"_SBRef_avg_brain.nii.gz"
SBRef_brain_mask=$sub_dir/$sub"_SBRef_avg_brain_mask.nii.gz"

#echo spacing for 7T_control_01-7T_control_08 0.000180001
#echo spacing for 7T_control_09-7T_control_21 0.000176666

spacing=0.000176666

echo "Subject ID: $sub"
echo "Echo spacing: $spacing"
...
epi_reg --epi=$SBRef_brain --t1=$t1_2mm --t1brain=$t1_brain_2mm --out=$sub_dir/$sub"_SBRef2struct" --fmap=$fmap --fmapmag=$fmap_mag --fmapmagbrain=$mag_brain --echospacing=$spacing --pedir=-y
fi
...
if [ -e $sub_dir/$sub"_epi_echo3_mc.nii.gz" ]; then
...
#apply registration to t1 and distortion correction warp to all echoes 
echo "Applying distortion correction and warping epi images to T1 2mm brain"
applywarp --ref=$t1_brain_2mm --in=$sub_dir/$sub"_epi_echo1_mc_st_brain.nii.gz" --warp=$sub_dir/$sub"_SBRef2struct_warp.nii.gz" --out=$sub_dir/$sub"_epi_echo1_preproc.nii.gz"
applywarp --ref=$t1_brain_2mm --in=$sub_dir/$sub"_epi_echo2_mc_st_brain.nii.gz" --warp=$sub_dir/$sub"_SBRef2struct_warp.nii.gz" --out=$sub_dir/$sub"_epi_echo2_preproc.nii.gz"
applywarp --ref=$t1_brain_2mm --in=$sub_dir/$sub"_epi_echo3_mc_st_brain.nii.gz" --warp=$sub_dir/$sub"_SBRef2struct_warp.nii.gz" --out=$sub_dir/$sub"_epi_echo3_preproc.nii.gz"
